...
# Code used to analyse results as given in the project found at
# https://doi.org/10.1002/adfm.202007086
...
transistor_name = 'Label 19 Med sq array T13'
#file_name = 'SD 20mV_100s.csv'
#file_name = 'EMG fas 500uV_1s_2.csv' 
file_name = 'ECoG 1mV_55s_2.csv'

if 'Label 1 ' in transistor_name:
    device_type = '70nm'
else:
    device_type = '35nm'
directory = ''
...
os.chdir(directory)
# file_name = 'amp_500uV f_10mA drain_-0.6 vg_0.csv'
transistor_name = '35nm EP'
device_type = '35nm'
neural_data = np.genfromtxt(file_name, delimiter=",", skip_header=1)
neural_data = neural_data[:,2:]
if 'Vd' in file_name:
    time_values = neural_data[:,2]
    gate_voltage = neural_data[:,0]
    drain_current = neural_data[:,4]
elif 'amp' in file_name:
    time_values = neural_data[:,1]
    gate_voltage = neural_data[:,0]
    drain_current = neural_data[:,3]
else:
    time_values = neural_data[:,1]
    gate_voltage = neural_data[:,0]
    drain_current = neural_data[:,2] 
...
#drain_current = drain_current - np.mean(drain_current)
#gate_voltage = gate_voltage - np.mean(gate_voltage)

if 'SD' in file_name:
    low_cutoff = 0.001
    butterworth_order = 1
    sig_type = 'Spreading Depolarisation'
elif 'fas' in file_name:
    low_cutoff = 10
    butterworth_order = 3
    sig_type = 'EMG Fasciculation'
elif 'ECoG' in file_name:
    low_cutoff = 1
    butterworth_order = 3
    sig_type = 'ECoG'
...
ax1.title.set_text('No Filter ' + sig_type + ' ' + device_type)
color = 'tab:blue'
...
fig, ax1 = plt.subplots()
ax1.title.set_text('Bandpass ' + sig_type + ' ' + device_type)
color = 'tab:blue'
ax1.set_xlabel('Time (s)')
#ax1.set_xlim([0.92,1.01])
ax1.set_ylabel('Gate Voltage (uV)', color=color)
...
#ax1.set_xlim([0.92,1.01])
ax1.tick_params(axis='y', labelcolor=color)
ax1.title.set_text('Notch/Bandpass Day 0 SC ' + sig_type + ' ' + device_type)
if 'SD' in file_name:
    ax1.plot(time_values, 1000*bandpassed_gate_voltage, color = color, label = 'Gate Voltage', alpha=0.7)
    ax1.set_ylabel('Gate Voltage (mV)', color=color)
else:
    ax1.plot(time_values, 1000000*bandpassed_gate_voltage, color = color, label = 'Gate Voltage', alpha=0.7)
...
def fft_plot(gate_voltage_values, drain_current_values, time_interval_value, sig_type_value, device_type_value, filter_type):
    ...
    ax1.set_yscale('log')
    ax1.set_title('Fourier Transform ' + sig_type_value + ' ' + device_type_value + ' ' + filter_type)
    ...
    plt.ylabel('Normalised Fourier Amplitude')
    plt.yscale('log')
    #plt.ylim([0,0.1])
    plt.xscale('log')
    plt.title('Fourier Transform ' + sig_type_value + ' ' + device_type_value + ' ' + filter_type)
    plt.plot(freq_dom, fft_gate, color='r', label = 'Gate FFT')
    plt.plot(freq_dom, fft_drain, color='b', alpha=0.6, label = 'Drain FFT')
    plt.legend()
    """
    
#fft_plot(gate_voltage, drain_current, time_interval, sig_type, device_type, 'No Filter')
#fft_plot(bandpassed_gate_voltage, bandpassed_drain_current, time_interval, sig_type, device_type, 'Bandpass')
#fft_plot(bandpassed_gate_voltage, notched_bandpassed_drain_current, time_interval, sig_type, device_type, 'Notch/Bandpass')
#fft_plot(gate_voltage, simulated_drain_current, time_interval, sig_type, device_type, 'No Filter')
#fft_plot(bandpassed_gate_voltage, simulated_bandpass_drain_current, time_interval, sig_type, device_type, 'Bandpass')
#fft_plot(bandpassed_gate_voltage, simulated_notched_bandpass_drain_current, time_interval, sig_type, device_type, 'Notch/Bandpass')
...
if simulate_oect == 'yes':
    ...
    """
    # Fasciculation 35
    optimal_sigma_value_no_filter = 34e-6
    optimal_sigma_value_bandpass = 2.8e-6
    optimal_sigma_value_notch = 0.35e-6
    
    # ECoG 35
    optimal_sigma_value_no_filter = 11e-6
    optimal_sigma_value_bandpass = 11e-6
    optimal_sigma_value_notch = 3e-6
    
    # SD 35
    optimal_sigma_value_no_filter = 13e-6
    optimal_sigma_value_bandpass = 13e-6
    optimal_sigma_value_notch = 5e-6
    
    # EMG 35
    optimal_sigma_value_no_filter = 14e-6
    optimal_sigma_value_bandpass = 14e-6
    optimal_sigma_value_notch = 7e-6
     
    # LFP 35
    optimal_sigma_value_no_filter = 19e-6
    optimal_sigma_value_bandpass = 19e-6
    optimal_sigma_value_notch = 12e-6
    
    """
    #optimal_sigma_value_no_filter, binned_none, trans_gat_dif_sim_no_filter = OECT_simulation(gate_voltage, raw_transconductance_mean, bin_no_filter, drain_current, 'No Filter', bin_width_value, optimal_sigma_value_no_filter, optimal_sigma_value_no_filter+1, 10)
    #optimal_sigma_value_bandpass, binned_bandpass, trans_gat_dif_sim_bandpass = OECT_simulation(bandpassed_gate_voltage, raw_transconductance_mean, bin_bandpass, bandpassed_drain_current, 'Bandpass', bin_width_value, optimal_sigma_value_bandpass, optimal_sigma_value_bandpass+1, 10)
    #optimal_sigma_value_notch, binned_notch, trans_gat_dif_sim_notch = OECT_simulation(bandpassed_gate_voltage, raw_transconductance_mean, bin_notch, notched_bandpassed_drain_current, 'Notch/Bandpass', bin_width_value, optimal_sigma_value_notch, optimal_sigma_value_notch+1, 10)
...
# gate_difference_values = [i[0] for i in trans_gate_dif_notch]
# transconductance_values = [i[1] for i in trans_gate_dif_notch]
# gate_difference_values_sim = [i[0] for i in trans_gat_dif_sim_notch]
# transconductance_values_sim = [i[1] for i in trans_gat_dif_sim_notch]
# # plot raw transconductance values
# plt.figure(figsize=(6.5,5))
# plt.title(sig_type +' Notch/Bandpass Transconductance')
# plt.xlabel('Gate Voltage Change ($\mu$V)')
...
# #plt.xlim([-350,350])
# #plt.ylim([-1001000,1001000])
# plt.scatter(1000000*np.array(gate_difference_values),1000*np.array(transconductance_values), c='r', marker = 'x', label = 'Measured')
# plt.scatter(1000000*np.array(gate_difference_values_sim),1000*np.array(transconductance_values_sim), c='b', marker = 'x', label = 'Simulated')
# plt.legend()

# # Noise sigma low pass
# low_pass_frequency = [5, 7.5, 10, 12.5, 15, 17.5, 20, 22.5, 25]
# noise_values = [12, 20, 32, 48, 64, 80, 96, 114, 132]
# plt.figure(figsize=(7.5,6))
# plt.title('Bandpass EMG Fasciculation 4$\sigma$ Noise for Varying Low-pass Frequencies')
# plt.ylabel('Optimal Simulated Noise Parameter, 4$\sigma$ ($\mu$V)')
# plt.xlabel('Low-pass Frequency (kHz)')
# plt.grid(True)
# plt.plot(low_pass_frequency, noise_values, 'x-')
